 ...
 *
 * Emitted from provider/manifest.json (via weaver's template/embed/ui/connector.ts) by weaver's generator.
 * A hand-edit here is destroyed by the next protocol sync, which is worse than
 * being rejected, because it works until it silently does not. Fix
 * provider/manifest.json (via weaver's template/embed/ui/connector.ts) (or weaver's template/) and regenerate:
 *
 ...
 *
 * This is the SINGLE SOURCE for the connector authoring layer, and it has no
 * published home yet. It lives here because:
 *
 *   - `@particle-academy/fancy-connector-core` deliberately excludes it. The
 *     runtime owns the wire; this imports `@particle-academy/fancy-flow/engine`
 *     to build a node's config schema, which is a fancy-flow concern.
 *   - `@particle-academy/fancy-flow` does not export it either.
 *   - The flow-node marketplace carries it as vendored source at
 *     `_connector/ui/connector.ts`, which a PACKAGE cannot import.
 *
 * So every generated `<provider>-ui` package carries a copy emitted from this
 * file, and `new-provider.mjs --check` fails CI when a copy differs. One source,
 * many verified copies — the arrangement `fancy-connector-core/scripts/
 * vendor.mjs` already uses for `_connector/`, and the check is the part that
 * makes the duplication safe rather than a faster way to drift.
 *
 * The permanent fix is a `./connectors` export on `@particle-academy/fancy-flow`.
 * When that lands, this file becomes a re-export and every provider picks it up
 * on the next protocol sync.
 */
